...
'''
FIRST_TABLE_NAME = 'movies'
SECOND_TABLE_NAME = 'rating'
SORT_COLUMN_NAME_FIRST_TABLE = 'MovieID1'
SORT_COLUMN_NAME_SECOND_TABLE = 'Rating'
JOIN_COLUMN_NAME_FIRST_TABLE = 'MovieID1'
...
def ParallelSort (InputTable, SortingColumnName, OutputTable, openconnection):
    ...
    cur.execute('CREATE TABLE IF NOT EXISTS {} AS TABLE {}'.format(OutputTable, InputTable))
    openconnection.commit()
...
def ThreadsWorking1 (maintable,partiton_table,ColumnName,mini,maxi,openconnection):
    openconnection.set_isolation_level(psycopg2.extensions.ISOLATION_LEVEL_AUTOCOMMIT)
    cur = openconnection.cursor()
    #print 'INSERT INTO {} SELECT * from {} where {}>{} AND {}<={} ORDER BY {}'.format(partiton_table,maintable,ColumnName,mini,ColumnName,maxi,ColumnName)
    if (mini != 1):
...
def ThreadsWorking2 (InputTable1, InputTable2, Table1JoinColumn, Table2JoinColumn, OutputTable1, openconnection):
    openconnection.set_isolation_level(psycopg2.extensions.ISOLATION_LEVEL_AUTOCOMMIT)
    cur = openconnection.cursor()
    query = 'CREATE TABLE IF NOT EXISTS {} AS SELECT * from {} INNER JOIN {} ON ({}.{} = {}.{})'.format(OutputTable1,InputTable1,InputTable2,InputTable1,Table1JoinColumn,InputTable2,Table2JoinColumn)
    #cur.execute('CREATE TABLE IF NOT EXISTS {} AS SELECT * {} INNER JOIN {} ON ({}.{} = {}.{})'.format(OutputTable1,InputTable1,InputTable2,InputTable1,Table1JoinColumn,InputTable2,Table2JoinColumn))
    #print query
    cur.execute(query)

    #print 'OutputTable1,InputTable1,InputTable2,InputTable1,Table1JoinColumn,InputTable2,Table2JoinColumn'
...
def ParallelJoin (InputTable1, InputTable2, Table1JoinColumn, Table2JoinColumn, OutputTable, openconnection):
    ...
    for i in range(1,5):
        value1 = min_value1 + part_size1 + value1
        parti1.append(value1)
    ...
    for i in range(1,5):
        value2 = min_value2 + part_size2 + value2
        parti2.append(value2)
    parti2.append(max_value2)
    #for i in range(5):
    cur.execute('CREATE TABLE IF NOT EXISTS {} AS SELECT * from {} INNER JOIN {} ON ({}.{} = {}.{})'.format(OutputTable,InputTable1,InputTable2,InputTable1,Table1JoinColumn,InputTable2,Table2JoinColumn))
    cur.execute('DELETE FROM {}'.format(OutputTable))
    for i in range(5):
        if (parti1[i] != 1 and parti2[i] != 1):

            cur.execute('INSERT INTO {} SELECT * from {} where {}>{} AND {}<={}'.format('InputTablePart1'+str(i),InputTable1,Table1JoinColumn,parti1[i],Table1JoinColumn,parti1[i+1]))
            cur.execute('INSERT INTO {} SELECT * from {} where {}>{} AND {}<={}'.format('InputTablePart2'+str(i),InputTable2,Table2JoinColumn,parti2[i],Table2JoinColumn,parti2[i+1]))
        else:
            cur.execute('INSERT INTO {} SELECT * from {} where {}>={} AND {}<={}'.format('InputTablePart1'+str(i),InputTable1,Table1JoinColumn,parti1[i],Table1JoinColumn,parti1[i+1]))
            cur.execute('INSERT INTO {} SELECT * from {} where {}>={} AND {}<={}'.format('InputTablePart2'+str(i),InputTable2,Table2JoinColumn,parti2[i],Table2JoinColumn,parti2[i+1]))
    threads = []
    for i in range(5):
        t = threading.Thread(target = ThreadsWorking2,args = ('InputTablePart1'+str(i),'InputTablePart2'+str(i),Table1JoinColumn, Table2JoinColumn, 'OutputTable'+str(i), openconnection,))
        threads.append(t)
        t.start()
    for i in range(5):
